...
\begin{center}
	\begin{tikzpicture}[
			node distance=0.6cm,
			startstop/.style={rounded rectangle, draw=brandblue, fill=lightblue!20, minimum width=3cm, minimum height=0.8cm, font=\bfseries\small},
			process/.style={rounded rectangle, draw=mediumgray, fill=softgray, minimum width=3cm, minimum height=0.8cm, font=\bfseries\small},
			card/.style={rounded rectangle, draw=accentblue, fill=lightblue!30, minimum width=2.8cm, minimum height=0.8cm, font=\bfseries\small},
			arrow/.style={thick,->,>=stealth, draw=brandblue}
		]

		\node (start) [startstop] {\faUser\ Visitor on Subscribe Page};

		\node (display) [process, below=1cm of start] {\faList\ Display Newsletter Cards};

		\node (card1) [card, below=1cm of display, xshift=-5.5cm] {\faAppleAlt\ Kost and Nutrition};
		\node (card2) [card, below=1cm of display, xshift=-1.8cm] {\faBrain\ Mindset};
		\node (card3) [card, below=1cm of display, xshift=1.8cm] {\faMicroscope\ Knowledge and Research};
		\node (card4) [card, below=1cm of display, xshift=5.5cm] {\faDumbbell\ Weekly Workouts};
		\node (card5) [card, below=2.5cm of display, xshift=0cm] {\faRobot\ AI Training with Jaine};

		\node (toggle1) [process, below=1cm of card1, xshift=0cm] {\faToggleOn\ Toggle On/Off};
		\node (toggle2) [process, below=1cm of card2, xshift=0cm] {\faToggleOn\ Toggle On/Off};
		\node (toggle3) [process, below=1cm of card3, xshift=0cm] {\faToggleOn\ Toggle On/Off};
		\node (toggle4) [process, below=1cm of card4, xshift=0cm] {\faToggleOn\ Toggle On/Off};
		\node (toggle5) [process, below=1.5cm of card5, xshift=0cm] {\faToggleOn\ Toggle On/Off};

		\node (validate) [process, below=2.5cm of display, xshift=-1cm] {\faCheck\ Validate Selection};

		\node (warning) [process, right=3cm of validate] {\faExclamationTriangle\ Select at least 1};

		\node (email) [process, below=1.5cm of validate, xshift=0cm] {\faAt\ Enter Email};

		\node (submit) [process, below=1.2cm of email] {\faPaperPlane\ Submit Subscription};

		\node (success) [startstop, below=1.2cm of submit] {\faSmile\ Welcome Aboard};
		...
		\draw[arrow] (validate) -- node[left] {None} (warning);
		\draw[arrow, dashed] (warning) -- ++(0,1.5cm) -| (display);
