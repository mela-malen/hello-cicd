...
% --- 80s NEON SPANDEX AEROBICS PALETTE ---
...
\definecolor{softbg}{RGB}{250, 250, 250}
\definecolor{darkgrey}{RGB}{45, 52, 54}
\definecolor{sweettea}{RGB}{244, 208, 63}
\definecolor{energypink}{RGB}{255, 105, 180}
\definecolor{mintfresh}{RGB}{0, 206, 209}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textcolor{neonpink}{\textbf{DePLOJ}}}
\fancyhead[R]{\texttt{Newsletter} - CM Corp}
\fancyfoot[C]{\thepage\ / \total{page}}
\fancyfoot[R]{\today\ | v1.0}
...
% --- HEADER STYLING ---
\titleformat{\section}
{\color{neonpink}\Huge\bfseries}
{\thesection}{1em}{}[\vspace{0.5cm}]

\titleformat{\subsection}
{\color{neoncyan}\LARGE\bfseries}
{\thesubsection}{1em}{}[\vspace{0.3cm}]

\titleformat{\subsubsection}
{\color{energypink}\Large\bfseries}
...
\newtcolorbox{insightbox}[1]{
  enhanced, breakable,
  colback=pastelpink!20, colframe=neonpink,
  coltitle=white,
...
% 2. STEP BY STEP - Cyan/Mint
\newtcolorbox{stepbox}[1]{
  enhanced, breakable,
  colback=pastelcyan!20, colframe=mintfresh!80!black,
  coltitle=white,
  fonttitle=\bfseries\large,
  title={\faListOl\ \ #1},
  arc=5mm, boxrule=1.5mm,
...
% 3. IMPORTANT - Red/Orange
\newtcolorbox{importantbox}[1]{
  enhanced,
  colback=pastelpink!15, colframe=energypink!80!red,
  coltitle=white,
  fonttitle=\bfseries\large,
...
% 4. TECHNICAL - Yellow/Orange
\newtcolorbox{techbox}[1]{
  enhanced,
  colback=sweettea!20, colframe=sweettea!60!orange,
  coltitle=white,
  fonttitle=\bfseries\large,
  title={\faCogs\ \ #1},
  arc=2mm, boxrule=1mm,
...
\newtcolorbox{featurebox}[1]{
  enhanced, breakable,
  colback=pastelcyan!30, colframe=neoncyan,
  coltitle=white,
  fonttitle=\bfseries\large,
  title={\faStar\ \ #1},
  arc=5mm, boxrule=1.5mm,
...
\title{
    ...
    \vspace{0.5cm}
    \Large \textit{Enterprise-Grade Flask Subscription Platform} \\
    \large \textcolor{neoncyan}{Project Documentation for CM Corp}
}
\author{\textbf{Team DePLOJ} \\
Arbiona, Claude, Fredrick, Martina, Mikaela, Nahrin}
\date{\textbf{Generated:} \today \quad \textbf{Version:} 1.0}
...
\begin{center}
	\begin{tikzpicture}
		\node[draw=neonpink, line width=2mm, rounded corners=1cm, fill=pastelpink!10, minimum width=8cm, minimum height=2.5cm]
		{\Large \faCloud \ \ \faEnvelope \ \ \faDocker \\[0.5cm] \large \textit{Enterprise-Grade Subscription Platform}};
	\end{tikzpicture}
\end{center}
\vspace{3cm}
\begin{center}
	\textbf{\textcolor{neoncyan}{CM Corp Project Documentation}}
\end{center}
\vfill
\begin{center}
	{\small \texttt{Newsletter v1.0 | Generated: \today}}
\end{center}
\newpage

\begin{center}
	\textbf{\LARGE \textcolor{neonpink}{Contents}}
\end{center}
\vspace{1cm}
\tableofcontents
\newpage
...
\begin{center}
	\begin{tabular}{@{}lccc@{}}
		...
		\midrule
		Uptime          & 99.9\%          & 99.99\%          & \textcolor{neonpink}{\checkmark} \\
		Response Time   & $<200$ms        & 45ms             & \textcolor{neonpink}{\checkmark} \\
		Build Time      & $<5$min         & 2min             & \textcolor{neonpink}{\checkmark} \\
		Deployment Time & $<10$min        & 3min             & \textcolor{neonpink}{\checkmark} \\
		\bottomrule
	\end{tabular}
\end{center}
\vspace{0.5cm}
...
\begin{insightbox}{Architecture Principles}
	\begin{enumerate}
		...
		\item \textbf{80s Neon Theme} - Retro-futuristic aesthetic with neon pink and cyan accents creates a distinctive visual identity
...
\begin{center}
	\begin{tabular}{@{}lllc@{}}
		...
		\midrule
		macOS       & Monterey (12.x+) & zsh/bash       & \textcolor{neonpink}{\checkmark} \\
		Linux       & CachyOS          & fish/zsh       & \textcolor{neonpink}{\checkmark} \\
		Windows     & 11               & PowerShell     & \textcolor{neonpink}{\checkmark} \\
		\bottomrule
	\end{tabular}
\end{center}
\vspace{1cm}
...
\begin{center}
	\begin{tikzpicture}[
			node distance=1.5cm,
			every node/.style={fill=pastelcyan!10, draw=neoncyan, rounded corners, minimum width=2.5cm, minimum height=0.8cm},
			arrow/.style={thick,->,>=stealth, draw=neonpink}
		]
...
\begin{center}
	\begin{tikzpicture}[
			node distance=0.8cm,
			box/.style={fill=pastelpink!10, draw=neonpink, rounded corners, minimum width=3cm, minimum height=0.8cm, font=\bfseries\small},
		...
		\draw[arrow, draw=neonpink] (users) -- node[left] {1:N} (subscribers);

		\draw[arrow, draw=neoncyan] (subscribers) -- node[above right] {M:N} (newsletters);

		\node (subs) at (0, -1.5) [box, draw=neoncyan, fill=pastelcyan!10] {\faArrowsH\ subscriptions};

		\draw[arrow, draw=neonpink, dashed] (subscribers) -- (subs);
...
\begin{center}
	\begin{tikzpicture}[
			node distance=0.3cm,
			startstop/.style={rectangle, rounded corners, fill=sweettea!20, draw=sweettea!60!black, minimum width=2.2cm, minimum height=0.7cm},
			arrow/.style={thick,->,>=stealth, draw=neonpink}
		]
...
\begin{center}
	\begin{tikzpicture}[
			node distance=0.3cm,
			visitor/.style={rectangle, rounded corners, fill=pastelcyan!20, draw=neoncyan, minimum width=2.5cm, minimum height=0.7cm},
			action/.style={rectangle, rounded corners, fill=pastelpink!20, draw=neonpink, minimum width=2.5cm, minimum height=0.7cm},
			arrow/.style={thick,->,>=stealth, draw=neonpink}
		]
...
\begin{center}
	\begin{tikzpicture}[
			node distance=0.4cm,
			funnel/.style={trapezoid, fill=pastelpink!20, draw=neonpink, minimum width=5cm, minimum height=0.8cm, trapezoid left angle=110, trapezoid right angle=70},
			drop/.style={rectangle, rounded corners, fill=pastelpink!10, draw=energypink, minimum width=4cm, minimum height=0.6cm},
			arrow/.style={thick,->,>=stealth, draw=neoncyan}
		]
...
\subsection{Newsletter Selection Journey}
...
\begin{stepbox}{Selection Flow}
	\begin{enumerate}
		\item \textbf{Card Display} - Visual cards display each newsletter with title, description, and category icon
		\item \textbf{Topic Toggle} - Users toggle each newsletter on or off based on interests
		\item \textbf{Validation} - System ensures at least one newsletter is selected
		\item \textbf{Email Entry} - Valid selection proceeds to email address input
		\item \textbf{Confirmation} - Submit subscription and receive welcome message
	\end{enumerate}
\end{stepbox}
...
\begin{center}
	\begin{tikzpicture}[
			node distance=0.4cm,
			card/.style={rectangle, rounded corners, fill=pastelcyan!15, draw=neoncyan, minimum width=3.5cm, minimum height=0.9cm, font=\bfseries\small},
			process/.style={rectangle, rounded corners, fill=pastelpink!20, draw=neonpink, minimum width=3cm, minimum height=0.9cm, font=\bfseries\small},
			arrow/.style={thick,->,>=stealth, draw=neonpink}
		]

		\node (display) [card] {\faList\ Display Cards};

		\node (kost) [card, below=0.8cm of display, xshift=-3.5cm] {\faAppleAlt\ Kost och Naringslara};
		\node (mindset) [card, below=0.8cm of display, xshift=0cm] {\faBrain\ Mindset};
		\node (research) [card, below=0.8cm of display, xshift=3.5cm] {\faMicroscope\ Kunskap och Forskning};

		\node (workouts) [card, below=1cm of kost, xshift=1.8cm] {\faDumbbell\ Veckans Pass};
		\node (jaine) [card, below=1cm of research, xshift=-1.8cm] {\faRobot\ Trana med Jaine};

		\node (validate) [process, below=1.5cm of mindset] {\faCheck\ Validate Selection};
		\node (email) [process, below=1cm of validate] {\faAt\ Email Form};
		\node (submit) [process, below=1cm of email] {\faPaperPlane\ Submit};

		\draw[arrow] (display) -- (kost);
		\draw[arrow] (display) -- (mindset);
		\draw[arrow] (display) -- (research);
		\draw[arrow] (kost) -- (workouts);
		\draw[arrow] (research) -- (jaine);
		\draw[arrow] (workouts) -- (validate);
		\draw[arrow] (mindset) -- (validate);
		\draw[arrow] (jaine) -- (validate);
		\draw[arrow] (validate) -- (email);
		\draw[arrow] (email) -- (submit);
...
\begin{center}
	\begin{tikzpicture}[
			node distance=0.3cm,
			admin/.style={rectangle, rounded corners, fill=sweettea!20, draw=sweettea!60!black, minimum width=2.5cm, minimum height=0.7cm},
			action/.style={rectangle, rounded corners, fill=pastelpink!20, draw=neonpink, minimum width=2.5cm, minimum height=0.7cm},
			arrow/.style={thick,->,>=stealth, draw=neonpink}
		]

		\node (login) [admin] {\faLock\ \textbf{Admin Login}};
		\node (dashboard) [action, below=0.8cm of login] {\faDashboard\ \textbf{Dashboard}};
		...
		\node (export) [admin, below=0.8cm of bulk] {\faExport\ \textbf{Export Emails}};
		\node (logout) [action, below=0.8cm of export] {\faSignOutAlt\ \textbf{Logout}};
...
\begin{stepbox}{Quick Start Guide}
	\begin{enumerate}
		\item Clone the repository:
		      \begin{scriptsize}
			      \begin{verbatim}
git clone https://github.com/deploj/newsletter.git
cd newsletter
\end{verbatim}
		      \end{scriptsize}
...
\begin{featurebox}{Key Takeaways}
	\begin{itemize}
		...
		\item \textbf{Enterprise Ready:} 99.99\% uptime, $<50$ms response time
	\end{itemize}
\end{featurebox}
...
\begin{center}
	\begin{tabular}{@{}ll@{}}
		...
		\midrule
		Repository        & \url{https://github.com/deploj/newsletter}         \\
		CI/CD Pipeline    & \url{https://github.com/deploj/newsletter/actions} \\
...
\vspace{0.5cm}
\begin{center}
	{\small \texttt{Newsletter v1.0 | CM Corp | Generated: \today}}
\end{center}
\vspace{0.5cm}
\begin{center}
	{\small \texttt{Newsletter v1.0 | CM Corp | Generated: \today}}
