...
\begin{center}
	\begin{tikzpicture}[
			node distance=0.5cm,
			card/.style={rectangle, rounded corners, fill=lightblue!15, draw=accentblue, minimum width=3.8cm, minimum height=1cm, font=\bfseries\small},
			process/.style={rectangle, rounded corners, fill=softgray, draw=mediumgray, minimum width=3.5cm, minimum height=1cm, font=\bfseries\small},
			arrow/.style={thick,->,>=stealth, draw=brandblue}
		]

		\node (display) [card] {\faList\ Display Cards};

		\node (kost) [card, below=1cm of display, xshift=-4cm] {\faAppleAlt\ Kost och Naringslara};
		\node (mindset) [card, below=1cm of display, xshift=0cm] {\faBrain\ Mindset};
		\node (research) [card, below=1cm of display, xshift=4cm] {\faMicroscope\ Kunskap och Forskning};

		\node (workouts) [card, below=1.2cm of kost, xshift=2cm] {\faDumbbell\ Veckans Pass};
		\node (jaine) [card, below=1.2cm of research, xshift=-2cm] {\faRobot\ Trana med Jaine};

		\node (validate) [process, below=1.8cm of mindset] {\faCheck\ Validate Selection};
		\node (email) [process, below=1.2cm of validate] {\faAt\ Email Form};
		\node (submit) [process, below=1.2cm of email] {\faPaperPlane\ Submit};
