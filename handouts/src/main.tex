...
\begin{landscape}
	\begin{center}
		\begin{tikzpicture}[
				node distance=0.6cm,
				startstop/.style={rounded rectangle, draw=brandblue, fill=lightblue!20, minimum width=2.8cm, minimum height=0.8cm, font=\bfseries\small},
				process/.style={rounded rectangle, draw=mediumgray, fill=softgray, minimum width=2.8cm, minimum height=0.8cm, font=\bfseries\small},
				card/.style={rounded rectangle, draw=accentblue, fill=lightblue!30, minimum width=2.5cm, minimum height=0.8cm, font=\bfseries\small},
				arrow/.style={thick,->,>=stealth, draw=brandblue}
			]

			\node (start) [startstop] {\faUser\ Visitor on Subscribe Page};

			\node (display) [process, right=1.5cm of start] {\faList\ Display Cards};

			\node (card1) [card, right=1.5cm of display, yshift=2.5cm] {\faAppleAlt\ Kost and Nutrition};
			\node (card2) [card, right=1.5cm of display, yshift=0cm] {\faBrain\ Mindset};
			\node (card3) [card, right=1.5cm of display, yshift=-2.5cm] {\faMicroscope\ Knowledge};
			\node (card4) [card, right=3.5cm of display, yshift=1.2cm] {\faDumbbell\ Weekly Workouts};
			\node (card5) [card, right=3.5cm of display, yshift=-1.2cm] {\faRobot\ AI Training};

			\node (toggle1) [process, right=1.5cm of card1] {\faToggleOn\ Toggle};
			\node (toggle2) [process, right=1.5cm of card2] {\faToggleOn\ Toggle};
			\node (toggle3) [process, right=1.5cm of card3] {\faToggleOn\ Toggle};
			\node (toggle4) [process, right=1.5cm of card4] {\faToggleOn\ Toggle};
			\node (toggle5) [process, right=1.5cm of card5] {\faToggleOn\ Toggle};

			\node (validate) [process, right=3cm of toggle2, yshift=0cm] {\faCheck\ Validate};

			\node (warning) [process, above=1.5cm of validate] {\faExclamationTriangle\ Select 1+};

			\node (email) [process, right=2cm of validate] {\faAt\ Enter Email};

			\node (submit) [process, right=1.5cm of email] {\faPaperPlane\ Submit};

			\node (success) [startstop, right=1.5cm of submit] {\faSmile\ Welcome};
			...
			\draw[arrow] (card1) -- (card4);
			\draw[arrow] (card3) -- (card5);
