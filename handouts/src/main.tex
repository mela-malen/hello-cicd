...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textcolor{brandblue}{\textbf{DePLOJ}}}
\fancyhead[R]{\texttt{Newsletter} - CM Corp}
\fancyfoot[C]{\thepage\ / \total{page}}
\fancyfoot[R]{\today\ | v1.0}
\fancyfoot[L]{\texttt{Build: \input{|"git rev-parse --short HEAD 2>/dev/null || echo 'dev'"}}}
...
\begin{center}
	\begin{tikzpicture}
		\node[draw=brandblue, line width=2mm, rounded corners=1cm, fill=lightblue!10, minimum width=8cm, minimum height=2.5cm]
...
\begin{center}
	\begin{tikzpicture}[
		...
		\node (subs) at (0, -1.5) [box, draw=accentblue, fill=lightblue!10] {\faArrowsH\ subscriptions};
...
\begin{center}
	\begin{tikzpicture}[
			node distance=0.4cm,
			funnel/.style={trapezoid, fill=lightblue!20, draw=brandblue, minimum width=5cm, minimum height=0.8cm, trapezoid left angle=110, trapezoid right angle=70},
...
\begin{center}
	\begin{tikzpicture}[
			node distance=0.8cm,
			startstop/.style={rounded rectangle, draw=brandblue, fill=lightblue!20, minimum width=3.2cm, minimum height=0.9cm, font=\bfseries\small},
			process/.style={rounded rectangle, draw=mediumgray, fill=softgray, minimum width=3.2cm, minimum height=0.9cm, font=\bfseries\small},
			card/.style={rounded rectangle, draw=accentblue, fill=lightblue!30, minimum width=3cm, minimum height=0.9cm, font=\bfseries\small},
			arrow/.style={thick,->,>=stealth, draw=brandblue}
		]

		\node (start) [startstop] {\faUser\ Visitor on Subscribe Page};

		\node (display) [process, below=1.2cm of start] {\faList\ Display Newsletter Cards};

		\node (card1) [card, below=1.2cm of display, xshift=-6cm] {\faAppleAlt\ Kost and Nutrition};
		\node (card2) [card, below=1.2cm of display, xshift=-2cm] {\faBrain\ Mindset};
		\node (card3) [card, below=1.2cm of display, xshift=2cm] {\faMicroscope\ Knowledge and Research};
		\node (card4) [card, below=1.2cm of display, xshift=6cm] {\faDumbbell\ Weekly Workouts};
		\node (card5) [card, below=3cm of display, xshift=0cm] {\faRobot\ AI Training with Jaine};

		\node (toggle1) [process, below=1.5cm of card1] {\faToggleOn\ Toggle On/Off};
		\node (toggle2) [process, below=1.5cm of card2] {\faToggleOn\ Toggle On/Off};
		\node (toggle3) [process, below=1.5cm of card3] {\faToggleOn\ Toggle On/Off};
		\node (toggle4) [process, below=1.5cm of card4] {\faToggleOn\ Toggle On/Off};
		\node (toggle5) [process, below=1.5cm of card5] {\faToggleOn\ Toggle On/Off};

		\node (validate) [process, below=3.5cm of display, xshift=-1.5cm] {\faCheck\ Validate Selection};

		\node (warning) [process, right=4cm of validate] {\faExclamationTriangle\ Select at least 1};

		\node (email) [process, below=2cm of validate, xshift=-1.5cm] {\faAt\ Enter Email};

		\node (submit) [process, below=1.5cm of email] {\faPaperPlane\ Submit Subscription};

		\node (success) [startstop, below=1.5cm of submit] {\faSmile\ Welcome Aboard};

		\draw[arrow] (start) -- (display);

		\draw[arrow] (display) -- (card1);
		\draw[arrow] (display) -- (card2);
		\draw[arrow] (display) -- (card3);
		\draw[arrow] (display) -- (card4);
		\draw[arrow] (display) -- (card5);

		\draw[arrow] (card1) -- (toggle1);
		\draw[arrow] (card2) -- (toggle2);
		\draw[arrow] (card3) -- (toggle3);
		\draw[arrow] (card4) -- (toggle4);
		\draw[arrow] (card5) -- (toggle5);

		\draw[arrow] (toggle1) -- (validate);
		\draw[arrow] (toggle2) -- (validate);
		\draw[arrow] (toggle3) -- (validate);
		\draw[arrow] (toggle4) -- (validate);
		\draw[arrow] (toggle5) -- (validate);

		\draw[arrow] (validate) -- node[above] {None} (warning);
		\draw[arrow, dashed] (warning) -- ++(0,2cm) -| (display);

		\draw[arrow] (validate) -- node[right] {1+} (email);
		\draw[arrow] (email) -- (submit);
		\draw[arrow] (submit) -- (success);
