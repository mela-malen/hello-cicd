...
\begin{landscape}
	\begin{center}
		\begin{tikzpicture}[
				node distance=0.4cm,
				startstop/.style={rectangle, rounded corners, draw=brandblue, fill=lightblue!20, minimum width=2.4cm, minimum height=0.55cm, font=\bfseries\tiny},
				process/.style={rectangle, rounded corners, draw=mediumgray, fill=softgray, minimum width=2.2cm, minimum height=0.5cm, font=\bfseries\tiny},
				card/.style={rectangle, rounded corners, draw=accentblue, fill=lightblue!30, minimum width=2cm, minimum height=0.5cm, font=\bfseries\tiny},
				arrow/.style={thick,->,>=stealth, draw=brandblue}
			]

			\node (start) [startstop] {\faUser\ Subscribe Page};

			\node (display) [process, below=0.6cm of start] {\faList\ Display Cards};

			\node (card1) [card, below=0.6cm of display, xshift=-3.5cm] {\faAppleAlt\ Kost/Nutr};
			\node (card2) [card, below=0.6cm of display, xshift=-1.2cm] {\faBrain\ Mindset};
			\node (card3) [card, below=0.6cm of display, xshift=1.2cm] {\faMicroscope\ Knowledge};
			\node (card4) [card, below=0.6cm of display, xshift=3.5cm] {\faDumbbell\ Workouts};
			\node (card5) [card, below=1.4cm of display, xshift=0cm] {\faRobot\ AI Training};

			\node (toggle1) [process, below=0.6cm of card1] {\faToggleOn\ Toggle};
			\node (toggle2) [process, below=0.6cm of card2] {\faToggleOn\ Toggle};
			\node (toggle3) [process, below=0.6cm of card3] {\faToggleOn\ Toggle};
			\node (toggle4) [process, below=0.6cm of card4] {\faToggleOn\ Toggle};
			\node (toggle5) [process, below=0.6cm of card5] {\faToggleOn\ Toggle};

			\node (validate) [process, below=1.4cm of card3, xshift=-2.5cm] {\faCheck\ Validate};

			\node (warning) [process, right=2cm of validate] {\faExclamationTriangle\ Select 1+};

			\node (email) [process, below=1cm of validate] {\faAt\ Email};

			\node (submit) [process, below=0.6cm of email] {\faPaperPlane\ Submit};

			\node (success) [startstop, below=0.6cm of submit] {\faSmile\ Welcome};
