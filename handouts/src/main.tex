...
\begin{landscape}
	\begin{center}
		\begin{tikzpicture}[
				node distance=0.5cm,
				startstop/.style={rectangle, rounded corners, draw=brandblue, fill=lightblue!20, minimum width=2.8cm, minimum height=0.7cm, font=\bfseries\scriptsize},
				process/.style={rectangle, rounded corners, draw=mediumgray, fill=softgray, minimum width=2.5cm, minimum height=0.65cm, font=\bfseries\scriptsize},
				card/.style={rectangle, rounded corners, draw=accentblue, fill=lightblue!30, minimum width=2.4cm, minimum height=0.65cm, font=\bfseries\scriptsize},
				arrow/.style={thick,->,>=stealth, draw=brandblue}
			]

			\node (start) [startstop] {\faUser\ Visitor on Subscribe Page};

			\node (display) [process, below=0.8cm of start] {\faList\ Display Cards};

			\node (card1) [card, below=0.8cm of display, xshift=-4.5cm] {\faAppleAlt\ Kost/Nutrition};
			\node (card2) [card, below=0.8cm of display, xshift=-1.5cm] {\faBrain\ Mindset};
			\node (card3) [card, below=0.8cm of display, xshift=1.5cm] {\faMicroscope\ Knowledge};
			\node (card4) [card, below=0.8cm of display, xshift=4.5cm] {\faDumbbell\ Workouts};
			\node (card5) [card, below=1.8cm of display, xshift=0cm] {\faRobot\ AI Training};

			\node (toggle1) [process, below=0.8cm of card1] {\faToggleOn\ Toggle};
			\node (toggle2) [process, below=0.8cm of card2] {\faToggleOn\ Toggle};
			\node (toggle3) [process, below=0.8cm of card3] {\faToggleOn\ Toggle};
			\node (toggle4) [process, below=0.8cm of card4] {\faToggleOn\ Toggle};
			\node (toggle5) [process, below=0.8cm of card5] {\faToggleOn\ Toggle};

			\node (validate) [process, below=1.8cm of card3, xshift=-3cm] {\faCheck\ Validate};

			\node (warning) [process, right=2.5cm of validate] {\faExclamationTriangle\ Select 1+};

			\node (email) [process, below=1.2cm of validate] {\faAt\ Email Form};

			\node (submit) [process, below=0.8cm of email] {\faPaperPlane\ Submit};

			\node (success) [startstop, below=0.8cm of submit] {\faSmile\ Welcome};
